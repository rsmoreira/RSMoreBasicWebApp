...
public class AccountControllerTest {
	...
	public void findAllBlogsForNonExistingAccount() throws Exception {
		...
		when(service.findBlogsByAccount(1L)).thenThrow(
				new AccountDoesNotExistException());

		mockMvc.perform(get("/rest/accounts/1/blogs")).andExpect(
				status().isNotFound());
	}
	...
	public void createBlogExistingBlogName() throws Exception {
		...
		// here you are 'mocking' a test behavior, so, with this code, this test
		// will
		// always throw a BlogExistsException.
		when(service.createBlog(eq(1L), any(Blog.class))).thenThrow(
		...
		mockMvc.perform(
				post("/rest/accounts/1/blogs").content(
						"{\"title\":\"Test Title\"}").contentType(
						MediaType.APPLICATION_JSON)).andExpect(
				status().isConflict()); // This is the kind of exception
										// expected.
										// Take a look on our ConflictException,
										// there
										// there is a @ResponseStatus(value=
										// HttpStatus.CONFLICT)
										// that tells to junit that
										// ConflicException is a
										// Http Conflict exception (Http
										// Status).
	}
	...
	public void createAccountNonExistingUsername() throws Exception {
		...
		/*
		 * This code below make Mockito can get the object passed on the
		 * request. So you can use this to verify if your 'password' (that we
		 * had troubles before) was sent correctly on the request.
		 */
